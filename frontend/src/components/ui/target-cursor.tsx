...
function TargetCursorInner({
    ...
    useEffect(() => {
        ...
        if (hideDefaultCursor) {
            ...
            // Also hide for all elements to avoid flickering
        ...
        const scrollHandler = () => {
            ...
            if (elementUnderMouse) {
                if (elementUnderMouse === activeTarget || elementUnderMouse.closest(targetSelector) === activeTarget) {
        ...
        return () => {
            ...
            const hideStyle = document.getElementById('hide-cursor-style');
    ...
    useEffect(() => {
        ...
        if (spinTl.current.isActive()) {
            spinTl.current.kill(); // Reset if duration changes
            spinTl.current = gsap
                .timeline({ repeat: -1 })
                .to(cursorRef.current, { rotation: '+=360', duration: spinDuration, ease: 'none' });
        }
